...
def get_valid_training_samples_for_one_test_sample_on_one_variable(df_training, df_test, test_time_index, variable, environmental_bin) :
    # catch key error on df_test : 
    try : 
        lte = df_test[variable].sel(time=test_time_index).values + environmental_bin[variable]
        gte = df_test[variable].sel(time=test_time_index).values - environmental_bin[variable]
    except KeyError :
        print('KeyError : variable {} not found in df_test'.format(variable))
        return False
...
def find_training_and_test_set(df, test_set_number, cluster_nb) :
    ...
    print('#####')
    print("start guessing valid training / test set with the following environmental bin :")
    print(str(environmental_bin))
    print(f'looking for {test_set_number} test samples in training set, divided in {cluster_nb} clusters ')
    print('#####')
    while not found and count < max_guesses_allowed:
        ...
        for test_time in df_test.time.values :
            ...
            list_var=''
            for envir_var in environmental_bin.keys() :
                df_valid = get_valid_training_samples_for_one_test_sample_on_one_variable(df_valid, df_test, test_time, envir_var, environmental_bin)
                list_var = list_var + ' & ' + envir_var
                # print('Nb samples with valid {} : {} '.format(envir_var, str(len(df_valid.time)) ))
...
def get_anomaly_list_from_df(df) :
    ...
    for i in range(len(df.time)):
        ...
        for key in anomaly_catalogue.keys():
            ...
            print('keys len :'+ str(len(anomaly_catalogue.keys())))
            if (not found_flag) :
...
def get_X_Y_train_test_from_xr_dataset(df, X_channel_list = ['tp', 'hs', 'mag10', 'theta10'], Y_channel_list = ['X_psd', 'Y_psd'], folder = None, test_nb=40, clusters = 3, cut_low_frequency=1/35) :

    # First split training and test data with customs requirement on the validity domain of a model.
    df_training_set, df_test_set = find_training_and_test_set(df, test_nb, clusters)
    ...
    print('# selected input channels are : {}'.format(str(Y_channel_list)))
    print('# channel training set numpy transformation success. Shape of channel training is {} '.format(str(np.shape(Y_numpy_channels_training_set))))
    Y_numpy_channels_test_set = get_numpy_input_channel_set(df_test_set, Y_channel_list)
    print('# channel test set numpy transformation success. Shape of channel test is {}'.format(np.shape(Y_numpy_channels_test_set)))
    ...
    # Prepare X
    # X_numpy_channels_training_set = get_numpy_input_channel_set(df_training_set, X_channel_list)
    # print('# selected input channels are : {}'.format(str(X_channel_list)))
    # print('# channel training set numpy transformation success. Shape of channel training is '.format(np.shape(numpy_channels_training_set)))
    # X_numpy_channels_test_set = get_numpy_input_channel_set(df_test_set, X_channel_list)
    # print('# channel test set numpy transformation success. Shape of channel test is '.format(np.shape(numpy_channels_test_set)))
    ...
    data = [X_numpy_channels_training_set, X_numpy_channels_test_set, Y_numpy_channels_training_set, Y_numpy_channels_test_set, X_channel_list, Y_channel_list]
    files = ['X_train', 'X_val', 'Y_train', 'Y_val', 'envir_list', 'spectrum_list']     
    i =0
    if folder == None :
        folder = r'C:\Users\romain.ribault\Documents\GitHub\subsee4d-mooring-monitoring\unscaled_data'
    for file in files :
...
def Scale_Xscalar_data(X_numpy_channels_training_set, X_numpy_channels_test_set, file_names= ['x_train_scaled', 'x_val_scaled'], folder = None) :
    ...
    if folder == None :
        folder = r'C:\Users\romain.ribault\Documents\GitHub\subsee4d-mooring-monitoring\scaled_data'
    file = os.path.join(folder, 'X_channels_scaler.dat')        
    with open(file, 'wb') as f :
        pickle.dump(scaler, f)
    
    x_scaler_path = file
    
    data = [x_numpy_channels_training_set, x_numpy_channels_val_set]
    ...
    for file in file_names :
        ...
        i+=1
    print('###')
    print('X scaled data saved in files : {}'.format(file_names))
    print('###')

    return x_numpy_channels_training_set, x_numpy_channels_val_set, x_scaler_path
...
def Scale_Yspectrum_data(Y_numpy_channels_training_set:np.array, Y_numpy_channels_test_set:np.array, file_names = ['y_train_scaled', 'y_val_scaled'], folder = None) -> np.array:    
    ...
    if folder == None :
        folder = r'C:\Users\romain.ribault\Documents\GitHub\subsee4d-mooring-monitoring\scaled_data'
    file = os.path.join(folder, 'Y_channels_scaler.dat') 
    with open(file, 'wb') as f :
        pickle.dump(Y_channel_scalers, f)
    y_scaler_path = file
    
    data = [y_numpy_channels_training_set, y_numpy_channels_val_set]
    i =0
    for file in file_names :
        file = os.path.join(folder, file)    
        with open(file, 'wb') as f:
            pickle.dump(data[i], f)
        i+=1
    return y_numpy_channels_training_set, y_numpy_channels_val_set, y_scaler_path
...
def PCA_Yspectrum_data(Y_numpy_channels_training_set:np.array, Y_numpy_channels_test_set:np.array, file_names = ['Y_train_pca', 'Y_val_pca'], folder = None, n_component=24) -> np.array:    
    ...
    if folder == None :
        folder = r'C:\Users\romain.ribault\Documents\GitHub\subsee4d-mooring-monitoring\scaled_data'
    file = os.path.join(folder, 'Y_channels_pca.dat') 
    with open(file, 'wb') as f :
        pickle.dump(Y_channel_pcas, f)
    Y_pca_path = file
    ...
    for file in file_names :
        ...
        with open(file, 'wb') as f:
            pickle.dump(data[i], f)
        i+=1
    return Y_numpy_channels_training_set, Y_numpy_channels_val_set, Y_pca_path